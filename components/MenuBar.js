...
const MenuItem = ({ href, children }) => (
  <Link href={href} className="group relative">
    <span className="font-medium transition-colors hover:text-accent">{children}</span>
...
const MobileMenuItem = ({ href, children, onClick }) => (
  ...
  >
    <Link href={href} onClick={onClick} className="block py-2 px-4 text-lg font-medium hover:bg-accent/10 rounded transition-colors">
      {children}
    </Link>
  </motion.div>
);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4 md:justify-start md:space-x-10">
          <div className="flex justify-start lg:w-0 lg:flex-1">
            <Link href="/">
              <img src="/ceewai_chi.svg" alt="logo icon" className="h-8 w-auto sm:h-10 bg-primary rounded-lg transition-transform hover:scale-105" />
            </Link>
          </div>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              <MobileMenuItem href="#about" onClick={toggleMenu}>Details</MobileMenuItem>
              <MobileMenuItem href="#prizes" onClick={toggleMenu}>Prizes</MobileMenuItem>
              <MobileMenuItem href="#sponsors" onClick={toggleMenu}>Sponsors</MobileMenuItem>
              <MobileMenuItem href="#timeline" onClick={toggleMenu}>Timeline</MobileMenuItem>
              <MobileMenuItem href="/rules" onClick={toggleMenu}>Rules</MobileMenuItem>
              <MobileMenuItem href="/privacy" onClick={toggleMenu}>Privacy Policy</MobileMenuItem>
            </div>
            <div className="pt-4 pb-3 border-t border-gray-200">
              <div className="flex items-center px-5">
                <div className="flex-shrink-0">
                  <img className="h-10 w-10 rounded-full" src="/smubia_logo.jpeg" alt="BIA logo" />
                </div>
                <div className="ml-3">
                  <div className="text-base font-medium text-primary">Brought to you by</div>
                  <a href="https://smubia.com/" target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-accent">SMU Business Intelligence & Analytics</a>
                </div>
              </div>
            </div>
          </motion.div>
