...
const Participants = () => {
  ...
  return (
    <div
      className="min-h-screen flex my-12 items-center justify-center px-4 sm:px-6 lg:px-8"
      style={{
        background:
          "radial-gradient(circle, rgba(var(--accent-rgb), 0.15) 0%, rgba(var(--accent-rgb), 0) 40%)",
      }}
    >
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between">
          ...
          <div className="w-full lg:w-1/2 text-center">
                ...
                <div
                  ...
                  style={{
                    transform: `translateY(${
                      (index - currentParticipant) * 100
                    }%)`,
                    opacity: index === currentParticipant ? 1 : 0,
                  }}
                >
                  <p className="text-xl sm:text-2xl lg:text-3xl font-semibold text-accent font-mono">
                    {participant}
                  </p>
                </div>
              ))}
            ...
            <p className="text-lg sm:text-xl lg:text-2xl text-primary">
              interested in supporting SMUBIA?
            </p>

            <p className=" text-lg sm:text-xl lg:text-2xl text-primary font-mono pt-10">
              {" "}
              You can reach out to us
              <br />
              <a
                href="mailto:bia@sa.smu.edu.sg"
                className="underline text-accent transition duration-200 hover:scale-105 font-bold"
              >
                here
              </a>
              !
